...
def close_connection(exception):
    connnection = getattr(g, '_connection', None)
...
def employer(employer_id):
    ...
    jobs = execute_sql('SELECT job.id, job.title, job.description, job.salary FROM job JOIN employer ON employer.id = job.employer.id WHERE employer.id = ?', ['employer_id'])
